...
<div id="snippet${index}">
    <div class="snippet-base">
        <fieldset>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <input name="${Constants.SNIPPET_TITLE}${index}" type="text" class="input-block-level input-xlarge"
                           data-required="true"
                           value="${snippet.title}"
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <textarea id="snippetCode${index}" name="${Constants.SNIPPET_CODE}${index}"
                              class="input-block-level"
                              style="height: 100px;"><c:out value="${snippet.code}"/></textarea>
                </div>
            </div>
            <div class="row-fluid">
                ...
                <select id="snippet${index}Language" name="${Constants.SNIPPETS_LANGUAGE}${index}" data-required="true"
                    ...
                    <option value=""></option>
                    <%--@elvariable id="codeLanguages" type="java.util.List<com.tcode.util.CodeInfo>"--%>
                    <c:forEach var="codeLang" items="${custom:codes()}">
                        <option value="${codeLang.cmFileName}=${codeLang.xtype}" ${snippet.xtype eq codeLang.xtype? "selected":""}>${codeLang.name}</option>
                    </c:forEach>
                </select>
                <button id="formatBtn${index}" type="button" class="btn btn-primary btn-small"
                        title="Formatting mostly for CSS, HTML, XML"
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <textarea name="${Constants.SNIPPETS_DESCRIPTION}${index}"
                              class="input-block-level">${snippet.description}</textarea>
                </div>
            </div>
        </fieldset>
        <div class="btn-group">
            <button id="removeSnippetBtn${index}" type="button" class="btn ">
                <div class="icon-trash"></div>
                <small>Remove snippet</small>
            </button>
        </div>
    </div>
    <hr>
    <script>
        $(function () {
            ...
            <c:if test="${snippet ne null}">
            <c:set var="xtype" value="${custom:cmFile(snippet.xtype)}"/>
            CodeMirror.autoLoadMode(editor${index}, '${xtype}');
            editor${index}.setOption("mode", '${xtype}');
            </c:if>
            ...
            $("#formatBtn${index}").click(function () {
                editor${index}.autoFormatRange({line: 0, ch: 0}, {
                    line: editor${index}.lineCount(),
                    ch: editor${index}.getLine(editor${index}.lineCount() - 1).length
                });
